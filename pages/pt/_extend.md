...
Para resumir, eu aconselharia usar o `@extend` apenas para manter relações entre seletores, se dois seletores são característicamente similares, então, este será o momento perfeito para estender. No entanto, se eles são irrelacionados, mas compartilham algumas regras, um `@mixin` pode ser melhor.
