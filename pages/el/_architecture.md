...
Στον προγραμματισμό ηλεκτρονικών υπολογιστών, τα glob patterns καθορίζουν σύνολα ονόματα αρχείων με χαρακτήρες μπαλαντέρ, όπως `*.scss`. Γενικά, το globbing είναι η επιλογή ενός σύνολο αρχείων βασιζόμενοι σε ένα expression αντί κάποιας λίστας ονομάτων αρχείων.
Όταν εφαρμόζεται στη Sass, σημαίνει το import partial αρχείων μέσα στο [αρχείο main](#main-file) με ένα glob pattern αντί του να τα απαριθμίσουμε μεμονωμένα. Αυτό θα έκανε το αρχείο main να μοιάζει κάπως έτσι:

{% include snippets/architecture/05/index.html %}

Η Sass δεν υποστηρίζει το file globbing απο μόνο του γιατί μπορεί να είναι μια επικίνδυνη λειτουργία γιατί στη CSS υπάρχει προτεραιότητα στην σειρά των δηλώσεων. Όταν κάνουμε import αρχείων δυναμικά (το οποίο συνήθως γίνεται με αλφαβητική σειρά), χάνετε ο έλεγχος της σειράς προτεραιότητας, το οποίο μπορεί να οδηγήσει σε παρενέργειες.

Κατόπιν αυτού, σε μια αυστηρή component-based αρχιτεκτονική με επιπλέον προσοχή στην μή διάρρευση κανενός style από ένα partial στο άλλο, η σειρά προτεραιότητας δεν πρέπει να έχει σημασία πια, το οποίο θα επέτρεπε τα glob imports. Αυτό θα καταστήσει ευκολότερη την πρόσθεση ή αφαίρεση partials γιατί δεν θα ήταν πλέον απαραίτητη η προσεκτικά ενημέρωση του αρχείου main.

Για τη χρήση Ruby Sass, υπάρχει ένα Ruby gem που ονομάζεται [sass-globbing](https://github.com/chriseppstein/sass-globbing) που επιτρέπει ακριβώς αυτή τη συμπεριφορά. Αν όμς χρησιμοποιείτε node-sass, μπορείτε να βασιστείτε είτε στο Node.js, είτε σε οποιοδήποτε build tool χρησιμοποιείτε για να κάνετε το compilation (Gulp, Grunt, etc.).
