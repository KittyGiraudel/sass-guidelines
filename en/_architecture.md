...
A component partial can include component-specific `$variables`, `%placeholders`, and even `@mixin`s and functions. Keep in mind, though, that you should avoid referencing (i.e. `@import`ing) component files from other component files, as this can make your project's dependency graph an unmaintainable tangled mess.
