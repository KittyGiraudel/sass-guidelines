...
Architecting a CSS project is probably one of the most difficult things you will have to do in a project’s life. Keeping the architecture consistent and meaningful is even harder.
 ...
 چارچوب بندی سی اس اس یک پروژه به احتمال زیاد یکی از سخت ترین کارهایی است که در طول حیات یک پروژه انجام خواهید داد. همچنین حفظ یکپارچگی و بامفهوم و با معنا بودن چارچوب مورد نظر امریست مشکل.
...
خوشبختانه یکی از مزیت های اصلی  استفاده از پیش پردازش گر CSS ، قدرت جداسازی کدها بدون تحت تاثیرقرار دادن کیفیت کدها به چندین فایل مختلف است (همانند کاری که دستور `@import` در CSS انجام می دهد).
باتشکر از Sass  که زحمت دستور`@import`  را بدوش می کشد، که کاملا روشی ایمن (و درواقع توصیه شده اس) برای استفاده چندین فایل به اندازه لازم، در توسعه می باشد که کلیه فایل ها در هنگام تولید به یک تک فایل CSS  تبدیل می کند.
